...
def _spherical_kmeans_single_lloyd(
    ...
    # init
    print(x_squared_norms)
    print(f"X: {X} \n x_2_norm: {x_squared_norms} \n init: {init} \n random_st: {random_state} \n sam_wei: {sample_weight} \n cent: {n_clusters}")
    ## TODO: ANUJ We added sample_weight
    kmean = KMeans()
    centers = kmean._init_centroids(
        X,  x_squared_norms, init, random_state, sample_weight, n_centroids=n_clusters
    )
    ...
    for i in range(max_iter):
        ...
        # if sp.issparse(X):
        #     centers = _k_means._relocate_empty_clusters_sparse(
        #         X, sample_weight, labels, n_clusters, distances
        #     )
        # else:
        #     centers = _k_means._relocate_empty_clusters_dense(
        #         X.astype(float),
